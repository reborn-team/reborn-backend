...
class MyWorkoutRepositoryTest {
    ...
    void sliceTest() {
        List<MyWorkoutResponse> result = workoutQuerydslRepository.pagingMyWorkoutWithSearchCondition(new WorkoutSearchCondition(null, null, null, null), member.getId());
    ...
    void pageTest() {
        ...
        Slice<MyWorkoutResponse> page = new Slice<>(result);
    ...
    @DisplayName("내 운동 목록을 조회하고 값이 10개면 true를 반환한다.")
    void pageSearchTest() {
        List<MyWorkoutResponse> result = workoutQuerydslRepository.pagingMyWorkoutWithSearchCondition(new WorkoutSearchCondition(null, null, "han", null), member.getId());
    ...
    void myWorkoutDto() {
        List<MyWorkoutResponse> myWorkoutResponse = workoutQuerydslRepository.getMyWorkoutDto(member.getId(), WorkoutCategory.BACK);

        assertThat(myWorkoutResponse.size()).isEqualTo(11);
    }
